...
public:
  ...
  void SearchGoal()
  {
    ...
    if (action_state_ != BehaviorState::RUNNING)
    {
      ...
      if (search_count_ == 5)
      {
        ...
        {
          auto dist_sq = std::pow(search_region_[i].pose.position.x - enemy_x, 2) + std::pow(search_region_[i].pose.position.y - enemy_y, 2);
  ...
  void BuffGoal()
  {
    ...
    if (action_state_ != BehaviorState::RUNNING)
    {
      ...
      //由于定位误差,可能机器人到达了指定位置,但不是实际的Buff位置,因此Buff_Point是一组点,可以让机器人微调位置
  ...
  void UpdateRobotPose()
  {
    ...
    try
    {
      ...
      //将tf::Stamped<tf::Pose>转变为geometry_msgs::PoseStamped
      tf::poseStampedTFToMsg(robot_tf_pose, robot_pose);
      //获取map坐标系下base_link坐标系下原点的位置,即机器人当前的位置

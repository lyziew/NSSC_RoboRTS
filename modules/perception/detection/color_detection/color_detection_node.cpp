...
ColorDetectionNode::ColorDetectionNode(std::string name) : node_state_(rrts::common::IDLE),
                                                           enemy_direction_(0),
  ...
  while (running_)
  {
    ...
    if (node_state_ == NodeState::RUNNING)
    {
      ...
      if (!src_.empty())
      {
        ...
        // 颜色分离
        auto binary_color_img = cv_toolbox_.DistillationColor(src_, enemy_color_, using_hsv_, enable_debug_);
        if (enable_debug_)
        {
          cv::imshow("DistillationColor", binary_color_img);
          cv::waitKey(1);
        }
        // 二值化
